...
const SYSTEM_PROMPT = `You are an expert AI Stock Market Assistant embedded in a stock portfolio management app focused on Indian (NSE/BSE) and global (US) markets.

You have deep knowledge of the following:

INDIAN STOCKS (NSE): RELIANCE.NS, TCS.NS, HDFCBANK.NS, ICICIBANK.NS, INFY.NS, HINDUNILVR.NS, SBIN.NS, BHARTIARTL.NS, ITC.NS, KOTAKBANK.NS, LT.NS, AXISBANK.NS, ASIANPAINT.NS, MARUTI.NS, SUNPHARMA.NS, TITAN.NS, NESTLEIND.NS, WIPRO.NS, POWERGRID.NS, NTPC.NS, ONGC.NS, BAJFINANCE.NS, BAJAJFINSV.NS, ADANIENT.NS, ADANIPORTS.NS, APOLLOHOSP.NS, DRREDDY.NS, CIPLA.NS, COALINDIA.NS, JSWSTEEL.NS, TATASTEEL.NS, HCLTECH.NS, DIVISLAB.NS, BRITANNIA.NS, EICHERMOT.NS, HEROMOTOCO.NS, HDFCLIFE.NS, SBILIFE.NS, TECHM.NS, GRASIM.NS, INDUSINDBK.NS, BAJAJ-AUTO.NS, BPCL.NS, IOC.NS, HINDALCO.NS, TATAMOTORS.NS, PIDILITIND.NS, ABB.NS, ACC.NS, AMBUJACEM.NS, BANKBARODA.NS, BERGEPAINT.NS, BOSCHLTD.NS, CANBK.NS, CHOLAFIN.NS, COLPAL.NS, DABUR.NS, DLF.NS, GAIL.NS, GODREJCP.NS, HAVELLS.NS, ICICIGI.NS, IGL.NS, INDIGO.NS, IRCTC.NS, JUBLFOOD.NS, LICHSGFIN.NS, LUPIN.NS, NMDC.NS, PAGEIND.NS, PETRONET.NS, PNB.NS, SAIL.NS, SRF.NS, TORNTPHARM.NS, TVSMOTOR.NS, VEDL.NS, VOLTAS.NS, BEL.NS, BHARATFORG.NS, BIOCON.NS, DEEPAKNTR.NS, FEDERALBNK.NS, GLENMARK.NS, HAL.NS, IDFCFIRSTB.NS, IEX.NS, JINDALSTEL.NS, JSWENERGY.NS, LAURUSLABS.NS, LTTS.NS, MANAPPURAM.NS, MAXHEALTH.NS, METROPOLIS.NS, NHPC.NS, OBEROIRLTY.NS, OFSS.NS, PIIND.NS, POLYCAB.NS, PVRINOX.NS, RECLTD.NS, SBICARD.NS, SUNTV.NS, TATACHEM.NS, TATACONSUM.NS, TRIDENT.NS

GLOBAL STOCKS (US): AAPL, MSFT, GOOGL, AMZN, META, NVDA, TSLA, BRK-B, JPM, V, MA, UNH, HD, PG, DIS, BAC, XOM, KO, PFE, PEP, CSCO, T, VZ, ADBE, NFLX, CRM, INTC, AMD, ORCL, IBM, QCOM, TXN, AVGO, COST, WMT, MCD, NKE, LLY, MRK, ABBV, TMO, DHR, PYPL, UBER, SHOP, SQ, SNOW, PLTR, ZM, DOCU, SPOT, DDOG, OKTA, NET, TEAM, ZS, MDB, CRWD, PANW, F, GM, RIVN, BA, GE, CAT, DE, HON, UPS, FDX, DAL, UAL, AAL, BKNG, ABNB, MAR, HLT, SBUX, YUM, CMG, CVS, UNP, NSC, CSX, ADSK, ANET, LRCX, KLAC, ASML, TSM, BABA, JD, PDD, BIDU, SONY, SAP, RIO, BHP, VALE, NIO, XPEV, LI, SE, MELI, GRAB, ETSY, EBAY, ROST, TJX, LOW, DG, DLTR, KR, TGT, BK, BLK, GS, MS, SCHW, AXP, C, USB, PNC, COF, MMM, RTX, LMT, GD, NOC
...
function ChatbotWidget() {
  ...
  async function sendMessage(text) {
    ...
    try {
      const reply = await callGeminiProxy(cappedHistory, SYSTEM_PROMPT)
      ...
      let msg
      if (status === 429) {
        msg = '⏳ Rate limit reached (15 req/min on free tier). Please wait ~60 seconds and try again.'
      } else if (detail) {
        msg = `⚠️ ${detail}`
      } else {
        msg = '⚠️ Unable to reach AI. Please try again.'
      }
